...
st.subheader("The following map plots all the buildings in the raw data based \
                on the filter below")
selectMapFilter = st.selectbox("Which feature would you like to use to filter?"\
                    ,("EUI","EnergyStar"))
placeholder1 = st.empty()
placeholder1.line_chart({"data": [1, 5, 2, 6]})
st.write("The building you have selected has a 380 EUI and energy star score \
            of 54 in the year 2020")
...
with col11:
    st.write("1.a. What are the trends in energy consumption of my selected \
            buildings across five years?")
    selectTrendFilter = st.selectbox("Which feature would you like to use to \
                                        compare your building?"\
                                        ,("Program Type","Zipcode"))
...
with col22:
    st.write("1.b. Comparison of energy sources of the building (steam, \
                electricity, natural gas) over the years")
    placeholder3 = st.empty()
    placeholder3.bar_chart({"data": [1, 5, 2, 6]})

##2nd chapter
st.subheader("2.Which characteristics of the building should be reviewed in \
                terms of energy efficiency?")

#placeholder to be replaced with visuals  
st.write("2.a. For the selected year how electricity intensive is each program \
            type?")

#############################################################################
# Electricity EUI by Program Type
#############################################################################

# Extracting the Required Columns and Renaming

dfEleProgram = df[["LargestPropertyUseType", "DataYear", "Electricity(kBtu)", 
                    "PropertyGFABuilding(s)"]]

dfEleProgram.rename(columns = {"DataYear" : "Year", 
                                "Electricity(kBtu)" : "Electricity", 
                                "PropertyGFABuilding(s)" : "BuildingArea", 
                                "LargestPropertyUseType" : "Program"}, 
                                inplace = True)

dfEleProgram.dropna(subset = ["Program", "Electricity"])

yearInput = st.selectbox("Year", dfEleProgram["Year"].unique())

dfEleProgram = dfEleProgram[dfEleProgram.Year == yearInput]

dfEleProgram = dfEleProgram.groupby(["Program"]) \
    .agg(
    totalElec = pd.NamedAgg(column = "Electricity", aggfunc = "sum"),
    totalArea = pd.NamedAgg(column = "BuildingArea", aggfunc = "sum"))

dfEleProgram = dfEleProgram.assign(ElectricityEUI = dfEleProgram.totalElec // 
                                    dfEleProgram.totalArea)

chart = alt.Chart(dfEleProgram.reset_index()).mark_bar().encode(
    x = "Program",
    y = "ElectricityEUI:Q",
    color = alt.Color("Program:N"), 
    tooltip = ["ElectricityEUI", "Program"]).properties(height = 750, 
                                                            width = 2000)
...
with col1:
    selectProgramFilter = st.selectbox("Select Program type",
                                        ("Program Type","Office"))
...
st.write(
    "The building you have selected has a 380 EUI and energy star score of 54 \
    in the comparison to a building with similar charecterists which has an \
    EUI of 200 and the building standard of 150")